...
(async () => {
  ...
  //    （這是同一個問題第四次過期：2026-06-24 第一次發現、2026-07-03 第二次發現並改成
  //    2026-07-02、2026-07-11 第三次發現並改成 2026-07-11。上次修正時特地把「目前確認的
  //    最新版本號」寫進這段註解裡（student.test.js v20、teacher.test.js v19、
  //    test-rules.js 100 條）想幫未來的自己核對，結果版本號本身也會過期——沒撐過幾天，
  //    2026-07-13～07-14 又新增 student.test.js→v23（S-SEC-32/33/34）、
  //    teacher.test.js→v22（T-17/T-17B/T-SEC-33/34/35）、test-rules.js→107 條
  //    （journalSubmitNotifiedAt 7 條），這裡完全沒跟著動，是 2026-07-15 這次稽核才發現。
  //    同一份 canary.yml 的 job 名稱這次也一併發現同步過期（見該檔案，已同步修正）。
  //    教訓：把「目前最新版本號」寫進提醒註解裡本身也不可靠，等於又製造一個要手動同步
  //    的地方；詳見 AI_測試架構說明.md 第四節陷阱17、AI_推播系統說明.md 第六節相關條目。）
  log(`對應 AI_CONTEXT.md 版本：2026-07-15`);
  log('═'.repeat(50));
