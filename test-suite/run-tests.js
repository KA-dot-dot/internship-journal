...
(async () => {
  ...
  //    這次（第七次，2026-07-25，對應「entriesCompleteAt 遲交判斷修正」補上自動化測試：
  //    student.test.js v25→v26 新增 S-SEC-40／S-SEC-41、teacher.test.js v25→v26 新增
  //    T-SEC-47）刻意不再重複這個做法，只更新日期本身。真要根治，仍建議改成執行時動態
  //    讀取 tests/*.test.js 開頭的版本注解，而不是任何形式的手動記憶，詳見
  //    AI_測試架構說明.md 第四節陷阱17。）
  log(`對應 AI_CONTEXT.md 版本：2026-07-25`);
  log('═'.repeat(50));
